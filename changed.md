...
## Lenovo Pen Plus Bluetooth Button Support (2026-07-07)

### Context
The Lenovo Pen Plus stylus button is **Bluetooth-based**, not a standard
HID stylus button. Investigation via
[lenovo-pen-plus-debug](https://github.com/ilamparithi-in/lenovo-pen-plus-debug)
showed that stock ZUI dispatches keycodes **600-604** (vendor-specific)
from a Bluetooth framework class (`BluetoothPenInputPolicy`) — not
`KEYCODE_STYLUS_BUTTON_PRIMARY/SECONDARY/TERTIARY/TAIL`.

| Keycode | clickStatusType | Gesture |
|---------|----------------|---------|
| 600 | 0 | Single press |
| 601 | 1 | Double press |
| 602 | 2 | Triple press |
| 603 | 3 | Long press |
| 604 | 4 | Long press + click |

### Changes Made

**`frameworks/base/services/core/java/com/android/server/policy/PhoneWindowManager.java`**
- Added a handler block in `interceptKeyBeforeQueueing()` (before the main
  key switch) that intercepts keycodes 600-604 when `mStylusButtonsEnabled=true`.
- On `ACTION_UP` (de-bounced, `repeatCount==0`) fires
  `lenovo.intent.action.INPUT_DEVICE_CLICK_STATE_CHANGED` broadcast with extras
  `DEVICE_TYPE=1`, `LENOVO_KEY_TYPE=clickStatusType`, `DEVICE_KEYTIMES_STATE=1`.
- Returns 0 (consumes event) so raw codes never reach foreground apps.

**`frameworks/base/core/res/AndroidManifest.xml`**
- Registered `lenovo.intent.action.INPUT_DEVICE_CLICK_STATE_CHANGED` as a
  `<protected-broadcast>` so only system processes can receive it securely.

---

## Freeform Window Management / Pop-up Mode (2026-07-07)

### Context
Stock ZUI supports pop-up windows and resizable app windows. This is
the Android Freeform Window Management feature.

### Changes Made

**`device/lenovo/tb351fu/overlay/frameworks/base/core/res/res/values/config.xml`**
- Added `config_freeformWindowManagement=true`
- Added `config_supportsMultiWindow=true`
- Restored accidentally dropped `config_sms_capable=false`

**`device/lenovo/tb351fu/device.mk`**
- Copies `frameworks/native/data/etc/android.software.freeform_window_management.xml`
  to `system/etc/permissions/` — this is the feature flag that
  `PackageManager.hasSystemFeature(FEATURE_FREEFORM_WINDOW_MANAGEMENT)` checks.
- Adds `persist.wm.extensions.enabled=true` — enables window decoration chrome
  (resize handles, drag bar) on freeform windows.
- Adds `ro.config.desktop_mode_supported=true` — advertises desktop mode to
  SystemUI so the taskbar shows the "Enter desktop mode" button.

---

## PenService / Stylus Support (2026-07-09)

### Problem
PenService APK was NOT installed on device despite being in `tb351fu-vendor.mk`
`PRODUCT_PACKAGES`. Root cause: APK was declared in both `system/priv-app`
and `vendor.mk`, but the build system ignored it because:
1. The APK path pointed to `proprietary/system/priv-app/PenService/` (system
   partition), but the Soong module had no `system_ext_specific: true`.
2. The `certificate: "platform"` was correct, but without the correct partition
   flag, the APK was not placed into `system_ext` where the system expected it.
3. The `privapp-permissions-penservice.xml` was shipped to
   `system/etc/permissions/` instead of `system_ext/etc/permissions/`.

### Changes Made

**`vendor/lenovo/tb351fu/Android.bp`**
- Changed APK path from `proprietary/system/priv-app/PenService/PenService.apk`
  to `proprietary/system_ext/priv-app/PenService/PenService.apk`
- Added `system_ext_specific: true` to the PenService module definition

**`device/lenovo/tb351fu/proprietary-files.txt`**
- Changed line from `system/priv-app/PenService/PenService.apk` to
  `system_ext/priv-app/PenService/PenService.apk`

**`device/lenovo/tb351fu/device.mk`**
- Added `PenService` to `PRODUCT_PACKAGES` explicitly
- Changed privapp permissions copy target from
  `$(TARGET_COPY_OUT_SYSTEM)/etc/permissions/` to
  `$(TARGET_COPY_OUT_SYSTEM_EXT)/etc/permissions/`

### Files Modified
- `vendor/lenovo/tb351fu/Android.bp` — PenService module (system_ext_specific)
- `device/lenovo/tb351fu/device.mk` — PRODUCT_PACKAGES + permissions path
- `device/lenovo/tb351fu/proprietary-files.txt` — extraction path

---

## Charging Control (Battery % Capping) via Lineage Health HAL (2026-07-09)

### Problem
`vendor.lineage.health.IChargingControl/default` service was not registered,
so the charging control UI never appears in Settings. The device's sysfs node
`/sys/devices/platform/charger/charging_enabled` exists and supports toggling.

### Changes Made

**`device/lenovo/tb351fu/BoardConfig.mk`**
- Added `soong_config_set` for Lineage Health HAL (post-July 2026 API,
  replacing old `TARGET_HEALTH_CHARGING_CONTROL_*` Make variables):
  ```
  ...
  ```
- These are read by `hardware/lineage/interfaces/health/aidl/default/Android.bp`
  via `soong_config_variable()` to define C preprocessor macros.
- Note: `charging_control_supports_toggle` is NOT set in BoardConfig.mk because
  its default in the HAL Android.bp is already `true`; setting it as a string
  via `soong_config_set` causes a type mismatch (the select expects bool).

**`device/lenovo/tb351fu/device.mk`**
- Added `vendor.lineage.health-service.default` to `PRODUCT_PACKAGES`

### Files Modified
- `device/lenovo/tb351fu/BoardConfig.mk` — soong_config_set entries
- `device/lenovo/tb351fu/device.mk` — PRODUCT_PACKAGES

---

## Offline Charger Animation (2026-07-09)

### Problem
When plugging in a powered-off device, the bootloader shows a static battery
icon instead of the LineageOS animated battery charging animation. The AOSP
`charger` binary exists at `/system/bin/charger` and animation resources are
present in `/product/etc/res/images/charger/`.

### Changes Made

**`device/lenovo/tb351fu/device.mk`**
- Added `ro.charger.enable_suspend=false` — prevents charger from suspending
  the display (keeps animation visible)
- Added `ro.charger.animate=true` — enables battery frame animation

### Files Modified
- `device/lenovo/tb351fu/device.mk` — system properties

---

## Advanced Protection Crash (2026-07-10)

### Problem
Opening Settings → Security & Privacy → Advanced Protection causes a
`SecurityException`:
```
java.lang.SecurityException: Access denied, requires: android.permission.MANAGE_ADVANCED_PROTECTION_MODE
at android.security.advancedprotection.AdvancedProtectionService.getAdvancedProtectionFeatures(...)
```
The permission `MANAGE_ADVANCED_PROTECTION_MODE` is gated by
`android:featureFlag="android.security.aapm_api"`. At runtime, `aconfigd-system`
crashes on boot (Rust OpenSSL linkage bug: missing symbol
`openssl::error::Error::fmt`), so the aconfig flag system fails and the
`android.security` package is never loaded → `ExportedFlags` throws
`ERROR_PACKAGE_NOT_FOUND` → permission denied.

### Changes Made

**`frameworks/base/core/res/AndroidManifest.xml`**
- Removed `android:featureFlag="android.security.aapm_api"` from both:
...
**`build/release/aconfig/bp4a/android.security/aapm_api_flag_values.textproto`**
- Created flag value file setting `aapm_api` to `ENABLED` with `READ_ONLY`
  permission (was missing from bp4a branch, existed in bp2a/trunk_staging)

### Files Modified
- `frameworks/base/core/res/AndroidManifest.xml` — removed featureFlag
- `build/release/aconfig/bp4a/android.security/aapm_api_flag_values.textproto` — new file

---

## Cross-Device Services Update Failure (2026-07-10)

### Problem
Play Store shows "Can't install Cross-Device Services" with error:
```
INSTALL_FAILED_UPDATE_INCOMPATIBLE: Existing package com.google.ambient.streaming
signatures do not match newer version; ignoring!
```
The APK was prebuilt in the GApps package at
`product/priv-app/CrossDeviceServices/CrossDeviceServices.apk` but was
**re-signed with the platform certificate** during build (`certificate: "platform"`).
When Play Store tries to install the official Google-signed update, the
signatures don't match.

### Changes Made

**`vendor/lenovo/tb351fu/Android.bp`**
- Changed `certificate: "platform"` to `presigned: true, preprocessed: true`
- `presigned: true` preserves the original Google signature
- `preprocessed: true` is required because the APK has `targetSdkVersion >= 30`
  (the build system would otherwise break the v2 signature)

### Files Modified
- `vendor/lenovo/tb351fu/Android.bp` — CrossDeviceServices module
